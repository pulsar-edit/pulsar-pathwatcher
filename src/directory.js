...
module.exports = class Directory {
  ...
  */
  ...
  //   (default: false)
  constructor(directoryPath, symlink1 = false, includeDeprecatedAPIs = Grim.includeDeprecatedAPIs) {
    this.symlink = symlink1;
  ...
  create(mode = 0o0777) {
    return this.exists().then((isExistingDirectory) => {
      ...
      if (this.isRoot()) {
        throw Error(`Root directory does not exist: ${this.getPath()}`);
      }
  ...
  */
  ...
  resolve(relativePath) {
    ...
    if (relativePath != null ? relativePath.match(/[A-Za-z0-9+-.]+:\/\//) : void 0) { // leave path alone if it has a scheme
      return relativePath;
  ...
  */
  ...
  getEntriesSync() {
    var directories, entryPath, files, i, len, ref, stat, symlink;
    directories = [];
    files = [];
    ref = fs.listSync(this.path);
